...
from esc.utils.wsd import expand_raganato_path, read_from_raganato, pos_map, WSDInstance, expand_data_path, read_from_chinese, cn_senses_from_dic
...
class QAExtractiveDataset(IterableDataset, ABC):
    ...
    def clean_dataset(self):
        ...
        if old_len != new_len:
            ...
            print(self.tokens_per_batch,self.tokenizer.model_max_length)
...
class ChineseDataset(QAExtractiveDataset):
    ...
    def __init__(
        ...
        if type(dataset_path) == str:
            dp,kp=expand_data_path(dataset_path)
            self.data_paths.append(dp)
            self.keys_paths.append(kp)
        else:
            for rp in dataset_path:
                dp,kp=expand_data_path(rp)
                self.data_paths.append(dp)
                self.keys_paths.append(kp)

        self.add_glosses_noise = add_glosses_noise
        self.senses_frequencies = None  # only used if the parameter "add_glosses_noise" is > 0.0
        self.fix_glosses = fix_glosses
        self.kshot = kshot
        self.poisson = poisson.Poisson(poisson_lambda)
        self.dic_path=dic_path

    def __init_senses_frequencies(self) -> None:

        senses_counter = collections.Counter()

        for data_path, keys_path in zip(self.data_paths, self.keys_paths):
            for wsd_sentence in read_from_chinese(data_path,keys_path):
                for wsd_instance in wsd_sentence:
                    if wsd_instance.labels is None:
                        continue
                    else:
                        cn_senses = [_l for _l in wsd_instance.labels]
                        senses_counter.update(cn_senses)

        total_occurrences=sum(senses_counter.values())

        self.senses_frequencies = [], []

        for cn_sense,count in senses_counter.items():
            self.senses_frequencies[0].append(cn_sense)
            self.senses_frequencies[1].append(count/total_occurrences)
    
    def init_dataset(self) -> None:

        self.data_store=[]

        if self.add_glosses_noise and self.senses_frequencies is None:
            self.__init_senses_frequencies()

        if self.kshot >0:
            senses_counter_k=collections.Counter()
            ...
            for wsd_sentence in read_from_chinese(data_path,keys_path):
                ...
                for i,wsd_instance in enumerate(wsd_sentence):
                    ...
                    if wsd_instance.labels is not None:
                        gold_labels = [_l for _l in wsd_instance.labels]
                        current_label = random.choice(gold_labels)

                    possible_senses= cn_senses_from_dic(wsd_instance.annotated_token.lemma,self.dic_path)

                    if len(possible_senses)==0:
                        print(
                            f"No synsets found in dic for instance {wsd_instance.instance_id}. "
                    ...
                    if not self.is_test:
                        ...
                        if self.kshot > 0:
                            if any([senses_counter_k[sk] > self.kshot for sk in wsd_instance.labels]):
                                continue
                            else:
                                senses_counter_k.update(wsd_instance.labels)
                            
                        if self.add_glosses_noise:
                            n_senses_to_add = self.poisson.sample().item()
                            senses,frequencies = self.senses_frequencies
                            random_senses=np.random.choice(
                                senses,size=int(n_senses_to_add), replace=False,p=frequencies
                            )
                            possible_senses += random_senses.tolist()

                        possible_senses=[
                            po for po in possible_senses if po == current_label or po not in gold_labels
                        ]

                        possible_senses=list(set(possible_senses))

                        if not self.fix_glosses:
                            np.random.shuffle(possible_senses)

                    curr_sentence = self.tokenizer.insert_classify_tokens(sentence_tokens,index=i)
                    possible_glosses=[
                        po + "。" for po in possible_senses
                    ]
                    ...
                    if current_label is not None:
                        try:
                            label_idx=possible_senses.index(current_label)
                        except ValueError:
                            print(
                                f"Instance {wsd_instance.instance_id} label ({current_label}) is not a valid label "
                                f"for sensedic given its word ({wsd_instance.annotated_token.lemma}) "
                                f"Skipping this instance"
                            )
                            continue
...
class MultiChineseDataset(QAExtractiveDataset):
    ...
    def __init_senses_frequencies(self) -> None:
        ...
        for data_path, keys_path in zip(self.data_paths, self.keys_paths):
            for wsd_sentence,_ in read_from_chinese(data_path,keys_path):
                for wsd_instance in wsd_sentence:
                    if wsd_instance.labels is None:
                        continue
                    else:
    ...
    def init_dataset(self) -> None:
        ...
        for data_path,keys_path in zip(self.data_paths,self.keys_paths):
            ...
            for wsd_sentence,del_lables in read_from_chinese(data_path,keys_path):
                ...
                for i,wsd_instance in enumerate(wsd_sentence):
                    ...
                    possible_senses= cn_senses_from_dic(wsd_instance.annotated_token.lemma,del_lables,self.dic_path)
